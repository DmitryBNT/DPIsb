...
void process_ndpi_collected_info(struct ndpi_workflow * workflow, struct ndpi_flow_info *flow) {
  ...
  if(flow->detected_protocol.app_protocol == NDPI_PROTOCOL_BITTORRENT) {
    ...
    printf("___________\n\n\n");
    memcpy(flow->http.ua,flow->ndpi_flow->http.ua,256);
  }
  /* MDNS */
...
static struct ndpi_proto packet_processing(struct ndpi_workflow * workflow,
  ...
  memcpy(flow->h_source,k1,6);
  memcpy(flow->h_dest,k2,6);
...
macs = (struct ndpi_ethhdr *) packet;
datalink_check:
...
iph_check:
  ...
  /* process the packet */
  return(packet_processing(workflow,macs->h_source,macs->h_dest, time, vlan_id, iph, iph6,
