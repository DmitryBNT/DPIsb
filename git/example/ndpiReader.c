...
static void printFlow(u_int16_t id, struct ndpi_flow_info *flow, u_int16_t thread_id) {
  ...
  if(!json_flag) {
    ...
    //'ether_ntoa(flow->h_source)
    fprintf(out, "%s%s%s:%u ( mac address = %s) %s %s%s%s:%u",
	    (flow->ip_version == 6) ? "[" : "",
	    flow->src_name, (flow->ip_version == 6) ? "]" : "", ntohs(flow->src_port),ether_ntoa((struct ether_addr*) flow->h_source),
	    flow->bidirectional ? "<->" : "->",
	    (flow->ip_version == 6) ? "[" : "",
	    flow->dst_name, (flow->ip_version == 6) ? "]" : "", ntohs(flow->dst_port)
      );
      fprintf(out,"( mac address = %s)\n",ether_ntoa( (struct ether_addr*) flow->h_dest));
    ...
    if(flow->detected_protocol.master_protocol) {
      ...
      fprintf(out, "[proto: %u.%u/%s]",
         ...
         if ((flow->detected_protocol.master_protocol)==NDPI_PROTOCOL_HTTP){
           fprintf(out,"   User-Agent:%s     ",flow->http.ua);
         }
    } else
      fprintf(out, "[proto: %u/%s]",
        ...
        if ((flow->detected_protocol.app_protocol)==NDPI_PROTOCOL_HTTP){
          fprintf(out,"   User-Agent:%s     ",flow->http.ua);
        }
