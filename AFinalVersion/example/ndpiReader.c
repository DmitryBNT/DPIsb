...
//Additional Libs and Variables
#include <malloc.h>
#include <time.h>
const char *name_of_pcap_file;
struct ndpi_flow_info *flow_addr;
void setName(char* pcap_file_name);
void moduleWork(char mw_source_ip[48], char mw_destination_ip[48], u_int16_t mw_source_port, u_int16_t mw_destination_port, int flag_work);
void changeFlow(struct ndpi_flow_info *flow);
...
static void printFlow(u_int16_t id, struct ndpi_flow_info *flow, u_int16_t thread_id) {
  ...
  if(!json_flag) {
    changeFlow(flow);
    moduleWork(flow->src_name, flow->dst_name, ntohs(flow->src_port), ntohs(flow->dst_port), 1);

    fprintf(out, "\t%u", id);

    fprintf(out, "\t%s ", ipProto2Name(flow->protocol));

    fprintf(out, "%s%s%s:%u (MAC Address: %s, Company for this MAC: %s) %s %s%s%s:%u (MAC Address: %s, Company for this MAC: %s)",
	    (flow->ip_version == 6) ? "[" : "",
	    flow->src_name, (flow->ip_version == 6) ? "]" : "", ntohs(flow->src_port),
      flow->mac_address_src, flow->company_src,
	    flow->bidirectional ? "<->" : "->",
	    (flow->ip_version == 6) ? "[" : "",
	    flow->dst_name, (flow->ip_version == 6) ? "]" : "", ntohs(flow->dst_port),
      flow->mac_address_dst, flow->company_dst
      );
    ...
    //moduleWork(flow->src_name, flow->dst_name, ntohs(flow->src_port), ntohs(flow->dst_port), 0);
    fprintf(out, "\n\nThis IP with the following User-Agent:\n%s\nmade an exchange through the HTTP protocol with the following IP with the following User-Agent:\n%s.\n",
      flow->user_agent_first,
      flow->user_agent_second
    );

    fprintf(out, "\nThe first activity of this pair was recorded at:\n%s\nThe last activity of this pair was recorded at:\n%s.\n",
      flow->time_first_act,
      flow->time_last_act
    );
...
moduleWork(flow->src_name, flow->dst_name, ntohs(flow->src_port), ntohs(flow->dst_port), 1);
    ...
    ///
    json_object_object_add(jObj,"host_a.mac",json_object_new_string(flow->mac_address_src));
    json_object_object_add(jObj,"host_a.company",json_object_new_string(flow->company_src));
    json_object_object_add(jObj,"host_b.user_agent",json_object_new_string(flow->user_agent_first));
    ///
    ...
    ///
    json_object_object_add(jObj,"host_b.mac",json_object_new_string(flow->mac_address_src));
    json_object_object_add(jObj,"host_b.company",json_object_new_string(flow->company_src));
    json_object_object_add(jObj,"host_b.user_agent",json_object_new_string(flow->user_agent_second));
    ///
    json_object_object_add(jObj,"pair_first.activity",json_object_new_string(flow->time_first_act));
    json_object_object_add(jObj,"host_last.activity",json_object_new_string(flow->time_last_act));
...
void setName(char* pcap_file_name)
{
  name_of_pcap_file = pcap_file_name;
}
...
void moduleWork(char mw_source_ip[48], char mw_destination_ip[48], u_int16_t mw_source_port, u_int16_t mw_destination_port, int flag_work)
{
  ...
  memset(flow_addr->user_agent_first, '\0', 100);
  memset(flow_addr->user_agent_second, '\0', 100);

  char errbuff[PCAP_ERRBUF_SIZE];

  pcap_t * pcap = pcap_open_offline(name_of_pcap_file, errbuff);
  ...
  {
      unsigned char *data_from_file_for_ip = (unsigned char*)data;

      int current_ip = 0;
      int number_ip_check = 2;
      int current_ip_position = 0;
      int number_of_pair_ip = 4;
      int offset_for_next = 0;
      int offset_start = 26;
      unsigned int pair_ip;

      char check_ip_addr_first[100], check_ip_addr_second[100], check_ip_addr_work[100];

      const char *dot_symbol = ".";

      memset(check_ip_addr_first, '\0', 100);
      memset(check_ip_addr_second, '\0', 100);
      memset(check_ip_addr_work, '\0', 100);

      for (current_ip = 0; current_ip < number_ip_check; current_ip++)
      {
      for (current_ip_position = 0; current_ip_position < number_of_pair_ip; current_ip_position++)
      {
      pair_ip = data_from_file_for_ip[offset_start+current_ip_position+offset_for_next];
      if (current_ip == 0)
      {
        sprintf (check_ip_addr_work, "%d", pair_ip);
        strcat(check_ip_addr_first,check_ip_addr_work);

        if (current_ip_position != (number_of_pair_ip-1))
        {
        strcat (check_ip_addr_first, dot_symbol);
        }
      }
      else
      {
        sprintf (check_ip_addr_work, "%d", pair_ip);
        strcat(check_ip_addr_second,check_ip_addr_work);
        if (current_ip_position != (number_of_pair_ip-1))
        {
        strcat (check_ip_addr_second, dot_symbol);
        }
      }
      }
      offset_for_next = 4;
      ...
      int current_port = 0;
      int number_port_check = 2;
      int current_port_position = 0;
      int number_of_pair_port = 2;
      int offset_for_next_port = 0;
      int offset_start_port = 34;
      unsigned int pair_port_fd, pair_port_sd, merge_fd_sd;

      char check_port_first[100], check_port_second[100], check_port_work[100];

      memset(check_port_first, '\0', 100);
      memset(check_port_second, '\0', 100);
      memset(check_port_work, '\0', 100);

      for (current_port = 0; current_port < number_port_check; current_port++)
      {
      pair_port_fd = data_from_file_for_ip[offset_start_port+offset_for_next_port];
      pair_port_sd = data_from_file_for_ip[offset_start_port+offset_for_next_port+1];
      if (current_port == 0)
      {
        merge_fd_sd = (pair_port_fd<<8)|(pair_port_sd);
        sprintf (check_port_work, "%d", merge_fd_sd);
        strcat(check_port_first,check_port_work);
      }
      else
      {
        merge_fd_sd = (pair_port_fd<<8)|(pair_port_sd);
        sprintf (check_port_work, "%d", merge_fd_sd);
        strcat(check_port_second,check_port_work);
      }
      offset_for_next_port = 2;
      }
	  
      unsigned char tip1 [100];
      unsigned char tp1 [100];
      unsigned char tip2 [100];
      ...
      unsigned char *us_src_ip = (unsigned char *)mw_source_ip;
      unsigned char *us_dst_ip = (unsigned char *)mw_destination_ip;

      unsigned char us_src_port[100];
      unsigned char us_dst_port[100];

      memset(us_src_port, '\0', 100);
      memset(us_dst_port, '\0', 100);

      sprintf (us_src_port, "%d", mw_source_port);
      sprintf (us_dst_port, "%d", mw_destination_port);

      strcpy(tip1,us_src_ip);
      strcpy(tip2,us_dst_ip);
      strcpy(tp1,us_src_port);
      strcpy(tp2,us_dst_port);

      int compare_ip_first = 1;
      int compare_ip_second = 1;
      int compare_port_first = 1;
      int compare_port_second = 1;

      compare_ip_first = strcmp (check_ip_addr_first, tip1);
      compare_ip_second = strcmp (check_ip_addr_second, tip2);
      compare_port_first = strcmp (check_port_first, tp1);
      compare_port_second = strcmp (check_port_second, tp2);

      int condition = 1;

      condition = (compare_ip_first||compare_port_first||compare_ip_second||compare_port_second);

      if (!condition == 1)
      {
      char buf[80];
      strftime(buf, sizeof(buf), "Day %-d, Month %B, Year %Y | Time %H:%M:%S", localtime(&header->ts.tv_sec));
      if (time_var == 0)
      {
      strcpy(flow_addr->time_first_act, buf);
      strcpy(flow_addr->time_last_act, flow_addr->time_first_act);
      time_var ++;
      }
      else
      {
      strcpy(flow_addr->time_last_act, buf);
      }

      int OUI = 0;
      int current_mac_address = 0;
      int number_mac_address = 2;
      int current_mac_address_position = 0;
      int number_of_octet = 6;
      int number_manufacturing_octet = 3;
      int offset_for_data = 0;
      char mac_address_first[100], mac_address_second[100], mac_address_work[100];
      char manufacturing_code_first[100],manufacturing_code_second[100], manufacturing_code_work[100];
      unsigned int octet;

      const char *colon = ":";
      unsigned char *data_from_file = (unsigned char*)data;

      memset(mac_address_first, '\0', 100);
      memset(mac_address_second, '\0', 100);
      memset(mac_address_work, '\0', 100);
      memset(manufacturing_code_first, '\0', 100);
      memset(manufacturing_code_second, '\0', 100);
      memset(manufacturing_code_work, '\0', 100);

      for (current_mac_address = 0; current_mac_address < number_mac_address; current_mac_address++)
      {
      for (current_mac_address_position = 0; current_mac_address_position < number_of_octet; current_mac_address_position++)
      {
        octet = data_from_file[current_mac_address_position+offset_for_data];
        if (current_mac_address == 0)
        {
          sprintf (mac_address_work, "%.2x", octet);
          strcat(mac_address_first,mac_address_work);
          if (current_mac_address_position != (number_of_octet-1))
          {
          strcat (mac_address_first, colon);
          }
          if (OUI < 3)
          {
            sprintf (manufacturing_code_work, "%.2x", octet);
            strcat(manufacturing_code_first,manufacturing_code_work);
            if (current_mac_address_position != (number_of_octet-4))
            {
            strcat (manufacturing_code_first, colon);
            }
            OUI++;
          }
          else
        ...
        {
          sprintf (mac_address_work, "%.2x", octet);
          strcat(mac_address_second,mac_address_work);
          if (current_mac_address_position != (number_of_octet-1))
          {
          strcat (mac_address_second, colon);
          }
          if (OUI < 3)
          {
            sprintf (manufacturing_code_work, "%.2x", octet);
            strcat(manufacturing_code_second,manufacturing_code_work);
            if (current_mac_address_position != (number_of_octet-4))
            {
            strcat (manufacturing_code_second, colon);
            }
            OUI++;
          }
          else
      ...
      strcpy(flow_addr->mac_address_src, mac_address_first);
      strcpy(flow_addr->mac_address_dst, mac_address_second);

      FILE *list_of_company;
      list_of_company = fopen("list_mac_addr_company", "rt");

      int length_of_str = 200;
      char data_from_loc[length_of_str];

      char * check_for_search;
      char * check_for_token;

      char search_first[200];
      char search_second[200];

      strcpy (search_first, manufacturing_code_first);
      strcpy (search_second, manufacturing_code_second);

      int f_company = 0;

      for (int cycle = 0; cycle < 2; cycle++)
      {
        while (!feof(list_of_company))
        {
          fgets (data_from_loc, length_of_str, list_of_company);
          if (cycle == 0)
          {
            check_for_search = strstr(data_from_loc, search_first);
          }
          else
          {
            check_for_search = strstr(data_from_loc, search_second);
          }
          if (check_for_search != NULL)
          {
            check_for_token = strtok (data_from_loc, " ");
            while (check_for_token != NULL)
            {
              check_for_token = strtok (NULL, " ");
              if (cycle == 0)
              {
                strcpy(flow_addr->company_src, check_for_token);
              }
              else
              {
                strcpy(flow_addr->company_dst, check_for_token);
              }
              check_for_token = strtok (NULL, " ");
            }
            f_company = 1;
        ...
        {}
        f_company = 0;
        fseek (list_of_company, 0 , SEEK_SET);
      }
      fclose(list_of_company);
    }

      unsigned char *data_from_file_ud = (unsigned char*)data;

      int condition_first = 1;
      int condition_second = 1;

      int compare_ip_fWf = 1;
      int compare_ip_fWs = 1;
      int compare_port_fWf = 1;
      int compare_port_fWs = 1;

      compare_ip_fWf = strcmp (check_ip_addr_first, tip1);
      compare_ip_fWs = strcmp (check_ip_addr_first, tip2);
      compare_port_fWf = strcmp (check_port_first, tp1);
      compare_port_fWs = strcmp (check_port_first, tp2);

      condition_first = (compare_ip_fWf||compare_port_fWf);
      condition_second = (compare_ip_fWs||compare_port_fWs);

      unsigned char user_agent[100];
      unsigned char user_agent_work[100];

      memset(user_agent, '\0', 100);
      memset(user_agent_work, '\0', 100);

      int check_memory = 0;
      int match_user = 0;
      unsigned int current_octet;
      char octet_data[100], octet_work_pair[100];

      memset(octet_data, '\0', 100);
      memset(octet_work_pair, '\0', 100);

      for(int all_pairs = 0; all_pairs < header->caplen; all_pairs++)
      {
        current_octet = data_from_file_ud[all_pairs];
        sprintf (octet_work_pair, "%.2x", current_octet);
        strcpy (octet_data,octet_work_pair);
        if(strcmp(octet_data, "55") == 0)
        {
          for (int i = all_pairs+1; i < all_pairs+4; i++)
          {
          current_octet = data_from_file_ud[i];
          sprintf (octet_work_pair, "%.2x", current_octet);
          strcat(octet_data,octet_work_pair);
          }
          if(strcmp(octet_data, "55736572") == 0)
          {
            int end_of_user;
            int incr = all_pairs;
            do
            {
            sprintf(user_agent_work, "%c", data_from_file_ud[incr]);
            if (check_memory > 90)
            {
            memset(user_agent_work, '\0', 100);
            }
            strcat(user_agent,user_agent_work);
            incr++;
            check_memory++;
        ...
        int check_inout;
        check_memory = 0;
        check_inout = (int)data_from_file_ud[all_pairs-1];

        if((strcmp(octet_data, "53") == 0) && check_inout != 77)
        {
          ...
          if (strcmp(octet_data, "536572766572") == 0)
          {
            int end_of_server;
            int incr = all_pairs;
            do
            ...
            incr++;
            check_memory++;
            end_of_server = (int)data_from_file_ud[incr];
            } while(end_of_server != 13);
          }
        }
      }

      if (!condition_first == 1)
      {
        if (strlen(user_agent) == 0)
        {
          if (strlen(flow_addr->user_agent_first) == 0)
          {
          strcpy(flow_addr->user_agent_first, "No data or data is corrupted");
          }
        }
        else
        {
          strcpy(flow_addr->user_agent_first, user_agent);
        }
      }

      if (!condition_second == 1)
      {
        if (strlen(user_agent) == 0)
        {
          if (strlen(flow_addr->user_agent_second) == 0)
          {
          strcpy(flow_addr->user_agent_second, "No data or data is corrupted");
          }
        }
        else
        {
          strcpy(flow_addr->user_agent_second, user_agent);
        }
      }

      int current_ip_address = 0;
      int number_ip_address = 2;
      int current_ip_address_position = 0;
      int number_of_pair = 4;
      int offset_for_ipdata = 0;
      int offset_for_block_of_data = 26;
      char ip_address_first[100], ip_address_second[100], ip_address_work[100];
      unsigned int pair;

      memset(ip_address_first, '\0', 100);
      memset(ip_address_second, '\0', 100);
      memset(ip_address_work, '\0', 100);

      for (current_ip_address = 0; current_ip_address < number_ip_address; current_ip_address++)
      {
      for (current_ip_address_position = offset_for_block_of_data; current_ip_address_position < number_of_pair + offset_for_block_of_data; current_ip_address_position++)
      {
        pair = data_from_file_ud[current_ip_address_position+offset_for_ipdata];
        if (current_ip_address == 0)
        {
          sprintf (ip_address_work, "%d", pair);
          strcat(ip_address_first,ip_address_work);
          if (current_ip_address_position != (number_of_pair + offset_for_block_of_data-1))
          {
          strcat (ip_address_first, dot_symbol);
          }
        }
        else
        {
          sprintf (ip_address_work, "%d", pair);
          strcat(ip_address_second,ip_address_work);
          if (current_ip_address_position != (number_of_pair + offset_for_block_of_data-1))
          {
          strcat (ip_address_second, dot_symbol);
          }
        }
      }
      offset_for_ipdata = 4;
  ...
  if (strlen(flow_addr->user_agent_second) == 0)
  {
  strcpy(flow_addr->user_agent_second, "This IP and Port were never a source");
  }
}
